...
{
    // 괄호 검사기
    internal class BracketChecker : StackAdapter<string>      // 스택 구현한거 상속
    {
        private StackAdapter<char> containerBracket;        // 입력값이 저장될 스택들
        private StackAdapter<char> inPutDataChar;              // 예비는 Pop으로 데이터를 삭제하며 스택을 확인할 때 쓰는 녀석인데 지금 상황에서는 
        ...
        public void InPut()                                 // 수식을 입력받아 스택에 저장합니다.
        {
            ...
            char[] inPutDataChar = new char[inPutData.Length];
            for(int i = 0; i < inPutData.Length; i++)
            {

                inPutDataChar[i] = inPutData[i];
                BracketCount(inPutData[i]);
                count++;
            }
            ...
            // string[] spareInPut = containerBracket.ToArray(); // 만들어진 스택을 복사해줍니다.
        }
        ...
        public bool SequenceCheck()                         // 괄호의 열고 닫음이 제대로 되었는지 확인하는 함수입니다.
        {
            ...
            {
                BracketCount(inPutDataChar.Pop());             // 스택에 가장 늦게 들어간, 수식의 맨 마지막에 있는 것부터 하나씩 확인해봅니다.
