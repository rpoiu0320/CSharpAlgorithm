...
namespace IteratorHomework
{
    ...
    public IEnumerator<T> GetEnumerator()
        ...
        {
            private LinkedList<T> list;             // 반복기가 가리킬 리스트
            private LinkedListNode<T> node;         // 반복기가 가리킬 노드
            private T current;                      // 노드의 데이터

            internal Enumerator(LinkedList<T> list) // 검색에 필요한 리스트와 노트 가져옴
            {
                this.list = list;                   
                this.node = list.head;              // 첫번째 노드부터 시작하도록
                this.current = default(T);          // default로 해서 무슨 형이 와도 대응
            }
            ...
            public bool MoveNext()      // 다음 노드로 이동
            {
                if (node != null)
                {
                    current = node.Value;       // 데이터를 current에 넣고
